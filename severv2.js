...
function connectWebSocket() {
    const ws = new WebSocket(SUNWIN_URL);

    ws.on("open", () => {
        console.log("🔗 WebSocket connected to Sunwin");
    });

    ws.on("message", (data) => {
        try {
            const parsedData = JSON.parse(data);
            handleMessage(parsedData); // Ensure this function is defined
        } catch (err) {
            console.error("Lỗi khi xử lý dữ liệu WebSocket:", err.message);
        }
    });
...
connectWebSocket(); // Start initial connection


setInterval(() => {
    broadcastPrediction(); // Ensure this function is defined and safe to call
}, 5000);
